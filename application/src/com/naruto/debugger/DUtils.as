package com.naruto.debugger
{
	...
	final public class DUtils
	{
		...
		public static function loadSettingValue(key:String,defaultValue:*):*
		{
			// Get the preferences
			var so:SharedObject = SharedObject.getLocal(DConstants.ID);
		...
		public static function saveSettingValue(key:String,value:*):void
		{
			// Get the preferences
			var so:SharedObject = SharedObject.getLocal(DConstants.ID);
			so.data[key] = value;
		...
		public static function loadWindowOptions(window:NativeWindow, id:String):void
		{
			...
			if (id == "com.qq.naruto.debugger.MainWindow" && so.data["nwCloseAction"] != null) {
				DData.nwCloseAction = int(so.data["nwCloseAction"]);
				DData.nwCloseAction = DData.A_MIN;
			}
		...
		public static function saveWindowOptions(window:NativeWindow, id:String):void
		{
			...
			if (!window.closed) {
				...
				if (id == "com.qq.naruto.debugger.MainWindow") {
					so.data["nwCloseAction"] = DData.nwCloseAction;
				}
