package com.naruto.debugger
{
	...
	internal class DConnectionMobile implements IConnection
	{
		...
		// Group pin
		private const PIN:String = "TranslationDebugger";
		...
		public function DConnectionMobile()
		{
			_connected = false;
			...
			switch (event.info.code)
			{
				...
				case "NetGroup.SendTo.Notify":
					if (event.info.from == _connectedNeighbor) {
						var item:DData = new DData(event.info.message.id, event.info.message.data);
						if (item.id != null) {
							DCore.handle(item);
						}
					}
		...
		public function send(id:String, data:Object, direct:Boolean = false):void {
			...
			if (direct && id == DCore.ID && _connected && _connectedNeighbor != null)
			{				
				// Get the data
				var obj:Object = {};
				obj["id"] = id;
		...
		public function connect():void {
			if (!_connected && _connection == null && TranslationDebugger.enabled) {
